...
function connect() {
    ...
    ws.onmessage = function(event) {
        ...
        if (noteObj['method'] == 'GET_BACK') {
            // Get notes from server only if localNotes don't already exist,
            // otherwise show localNotes
            var localNotesStr = localStorage.getItem("localNotes");
            if (localNotesStr === null) {
                ...
                displayAllNotes(JSON.parse(localNotesStr));
            }
        } 
        else if (noteObj['method'] == 'SYNC_BACK') {
            // Overwrite localNotes
            var noteList = noteObj['note_list'];
            localStorage.setItem("localNotes", JSON.stringify(noteList));
            displayAllNotes(noteList);
...
function submitNote(event) {
    ...
    // Add new note to local storage and try to sync
    ...
    // Prepend to local storage for later sync, also prepend to note list on canvas
    var localNotesObj = JSON.parse(localStorage.getItem("localNotes"));
    localNotesObj.unshift(noteObj);
    localStorage.setItem("localNotes", JSON.stringify(localNotesObj));
    displayNote(noteObj, true);
    // syncNotes();
...
function syncNotes() {
    // If localNotes exists, synchronize notes
    var localNotesStr = localStorage.getItem("localNotes");
    if (localNotesStr !== null) {
        var noteListToSync = [];
        var localNotesObj = JSON.parse(localNotesStr);
        for (var note of localNotesObj) {
            // Check for key in object, method can be POST, EDIT, DELETE
            if ("method" in note) {
                noteListToSync.push(note);
            }
        }
        ws.send(JSON.stringify({
            'method': 'SYNC',
            'note_list': noteListToSync
        }));
    }
}
